...
_SECRET_WARNING = """\
> [!WARNING]
> **This pull request does not remediate the exposure.**
>
> The credential was committed, so it is still in this repository's git history
> and still live whether or not this merges. Removing it from the working tree is
> cleanup, not remediation — and the diff below necessarily shows the value being
> removed.
>
> **Rotate the credential first.** Revoke the old value at its provider, confirm
> the replacement works, and only then merge this. Until rotation happens, treat
> the secret as compromised.

"""

# The one step that actually fixes a secret finding. Generated deterministically
# rather than left to the impact model, which may or may not think of it — and it
# rides along to the webhook, so the Slack message carries it too.
_ROTATE_STEP = "Rotate the exposed credential at its provider, then verify the old value is revoked"


def _with_rotation_step(impact: ImpactResult | None, feature_type: str) -> ImpactResult | None:
    """Put rotation at the top of the manual steps for a secret finding."""
    if impact is None or feature_type != "secret":
        return impact
    steps = list(impact.manual_steps or [])
    if not any("rotat" in step.lower() for step in steps):
        impact.manual_steps = [_ROTATE_STEP, *steps]
    return impact
...
def _build_pr_body(task: AlertTask, impact: ImpactResult | None) -> str:
    ...
    warning = _SECRET_WARNING if task.feature_type == "secret" else ""
...
def _notify_payload(task: AlertTask) -> NotificationPayload:
    """Build the payload for one alert, scrubbed.

    Every notification in the run is built here, which makes this the single
    place a credential could reach LogFileNotifier's file on disk or
    WebhookNotifier's POST to Slack. Scrubbing at the source means a backend
    ...
    return NotificationPayload(
        ...
        repo="",
        pr_url=task.pr_url,
        reason=redactor(task.failure_reason),
        impact_level=task.impact.level if task.impact else None,
        manual_steps=redactor.scrub(task.impact.manual_steps if task.impact else []),
        concerns=redactor.scrub(task.impact.concerns if task.impact else []),
        error_detail=redactor(task.impact.error if task.impact else None),
    )
...
def _run_pipeline(task: AlertTask, dry_run: bool, notifier, repo: Repository) -> AlertTask:
    ...
    if dry_run:
        ...
        p = _notify_payload(task)
        p.repo = repo.name
        # The fix agent's own account of what it would do. For a secret finding
        # that prose can quote the credential, and dry-run still notifies.
        p.detail = build_redactor(task.alert_json)(fix_result.diff_summary or "")
    ...
    task.impact = _with_rotation_step(
        analyze_impact(task.alert_json, diff_text,
